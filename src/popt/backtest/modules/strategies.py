...
from popt.backtest.modules.backtestdata import DataView, DataBuilder
from popt.backtest.modules.trading import TradingPolicy
from typing import Sequence

class BacktestStrategy:
    def __init__(self, dv: DataView, tp: TradingPolicy):
        assert isinstance(dv, DataView), type(dv)
        assert isinstance(tp, TradingPolicy), type(tp)
        self.dv = dv
        self.tp = tp
...
class MetaStrategy(BacktestStrategy):
    def __init__(
        ...
        tickers = np.unique([t for s in strategies for t in s.dv.tickers]).tolist()
        super().__init__(DataView(db=db, tickers=tickers))
        
        S = len(strategies) # FIXME different trading policies 
        ...
        for s in range(self.S):
            t2i = {t: i for i, t in enumerate(self.dv.tickers)}
            i_T = np.array([t2i[t] for t in self.strategies[s].dv.tickers])
...
class Markowitz(BacktestStrategy):
    def __init__(
        ...
        super().__init__(dv, tp)
        assert lev >= 0.0, lev
        assert len(w_max) == dv.N, len(w_max)
    ...
    def get_weights(self, t:int, w_prev:np.ndarray) -> np.ndarray:
        assert w_prev.shape == (self.dv.N,), w_prev.shape
        
        asset_mask  = self.dv.get_asset_mask(t-1)
        asset_mask &= self.dv.get_asset_mask(t)  # check for discontinuation
        w_max = self.w_max.copy()
        w_max[~asset_mask] = 0.0

        w: np.ndarray = markowitz(
            at     = self.dv.get_alpha(t-1),
            Ft     = self.dv.get_F_cov(t-1),
            dt     = self.dv.get_d_var(t-1),
            w_prev = w_prev, 
            w_max  = w_max, 
            gamma  = self.gamma, 
            lev    = self.lev, 
...
class FixedWeights(BacktestStrategy):
    def __init__(
        ...
        assert w_rebal.shape == (dv.N,), w_rebal.shape
    ...
    def get_weights(self, t:int, w_prev:np.ndarray) -> np.ndarray:
        assert w_prev.shape       == (self.dv.N,), w_prev.shape
        assert self.w_rebal.shape == (self.dv.N,), self.w_rebal.shape
        asset_mask  = self.dv.get_asset_mask(t-1)
        asset_mask &= self.dv.get_asset_mask(t)  # check for discontinuation
        Ft = self.dv.get_F_cov(t-1)
        dt = self.dv.get_d_var(t-1)
...
class MinimumVolatility(BacktestStrategy):
    def __init__(
        ...
        super().__init__(dv, tp)
        assert w_max.shape == (dv.N,), w_max.shape
        assert ((0.0 <= w_max) & (w_max <= 1.0)).all()
        self.w_max = w_max
    ...
    def get_weights(self, t:int, w_prev: np.ndarray) -> np.ndarray:
        ...
        if mask.sum() == 0:
            return np.zeros(self.dv.N)
        
        Ft = self.dv.get_F_cov(t-1)
        dt = self.dv.get_d_var(t-1)
        sigma = Ft @ Ft.T + np.diag(dt)  # covariance matrix sigma

        w = cp.Variable(self.dv.N, nonneg=True)
        prob = cp.Problem(
...
class MaximumDiversification(BacktestStrategy):
    ...
    def get_weights(self, t:int, w_prev: np.ndarray) -> np.ndarray:
        ...
        y = cp.Variable(self.dv.N, nonneg=True)
        prob = cp.Problem(
...
class RiskParity(BacktestStrategy):
    ...
    def get_weights(self, t:int, w_prev: np.ndarray) -> np.ndarray:
        ...
        Ft_n = self.dv.get_F_cov(t-1)[asset_mask]
        dt_n = self.dv.get_d_var(t-1)[asset_mask]
...
def asset_plot(
    ...
    ret = dv._ret.copy()
    ret[~dv._asset_mask] = 1.0
    ret_ = ret[window]

    plt.figure(figsize=figsize)
    plt.plot(timeline_, ret_.cumprod(axis=0)-1.0, label=dv.tickers)
    plt.legend()
    plt.show()
